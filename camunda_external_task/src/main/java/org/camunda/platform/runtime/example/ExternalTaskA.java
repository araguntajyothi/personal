...
@Component
@ExternalTaskSubscription("externalTaskA")
public class ExternalTaskA implements ExternalTaskHandler {
  ...
  public void execute(ExternalTask externalTask, ExternalTaskService externalTaskService) {
      try {
          ...
          System.out.println("External Task A Completed");
      } catch (Exception e) {
          // Configure retries with a time interval of 1 minute between each retry
        int retries = 3;
        long retryTimeout = 60000; // 1 minute in milliseconds
          externalTaskService.handleFailure(externalTask,"ERROR_A",e.getMessage(),retries, retryTimeout);   
      }
  }
  }
