...
@Component
@ExternalTaskSubscription("externalTaskC")
public class ExternalTaskC implements ExternalTaskHandler {

  @Override
  public void execute(ExternalTask externalTask, ExternalTaskService externalTaskService) {
      try {
          // Get the id name from the task variables
          ...
          System.out.println("External Task C Completed");
      } catch (Exception e) {
        int retries = 3;
        long retryTimeout = 60000; // 1 minute in milliseconds
          externalTaskService.handleFailure(externalTask,"ERROR_C",e.getMessage(),retries, retryTimeout); 
      }
  }
}